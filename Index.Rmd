...
(Check the FAQ page for more inforamtion)
...
I don't take exactly the same route in this video as I work through in the solutions. In the video I am demonstrating a method for doing the whole thing in one piped sequence. Which means there is one step in the middle which is a little more complex than the route outlined here in the solution. The code for the video solution is included at the end of this page as an appendix.
...
We need `dplyr` and `ggplot2`. So let's make sure we remember to load them in to our session!

We should also have probably remembered to set up a new project for this task as well.
...
We can import the data in a fairly standard way, using read.csv, or the "import dataset" menu
...
Then from zero we want an additional gradient to go from white to dark red as numbers become more positive. So the function we need is `scale_fill_gradient2`. In this we need to set the colour values for our "low", "mid" and "high" extremes of colour. 
...
An even better option here would be to use the new function `scale_fill_fermenter()`which works in a similar way to `scale_fill_brewer()` except with continuous variables being used to set colours instead. 
This is nice because the "RdBu" colour palette from colour brewer is exactly the colour palette used in the warming stripes plots.
...
A little surprisingly, because of the default options within `scale_fill_gradient2`, we now do have a symmetrical colour palette! The function defaults to a midpoint of zero, and stretches the colour assignments so that they are symmetrical. You can't tell this from just looking at the plot, but the top level of 'reds' being plotted is not at the full extent of 'darkred' unlike the blue which does extend as far as possible. If we did want to make sure of this explicitly we could set the limits within the scale call, exactly as we might modify the limits of an x or y axis as we have seen previously.
...
It may not be obvious without zooming in, but there are small spaces between each bar. This is because by default R sets bars to fill 90% of the width available, and leave 5% spacing between bars. However no such spaces exist in the 'classic' warming stripes. Thankfully this is easy to resolve by adding the option width=1 into geom_col.
...
This is the same route used in the video solution.
